...
export default function LecturerDetail() {
  ...
  const fetchLecturerDetail = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/lecturer/${userId}`);
      if (res.data.success) {
        setLecturer(res.data.data);
      }
    } catch (err) {
      console.log('Lecturer detail error:', err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Loading Lecturer...</Text>
      </View>
    );
  }
  ...
  const isActive   = lecturer.isActive   !== false;
  const isVerified = lecturer.isVerified === true;
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        >
          ...
          <View style={styles.profileCard}>
            ...
            <View style={styles.avatarWrap}>
              {lecturer.profileImage ? (
                <Image
                  source={{ uri: lecturer.profileImage }}
            ...
            <Text style={styles.profileId}>{lecturer.lecturerId}</Text>
            ...
            <View style={styles.chipRow}>
              ...
              <View style={[
                ...
                <MaterialCommunityIcons
                  ...
                  color="#002366"
                />
                <Text style={[styles.chipText, { color: '#002366' }]}>
                  {lecturer.courses?.length || 0} Course
                  {lecturer.courses?.length !== 1 ? 's' : ''}
                </Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.infoGrid}>
            <InfoTile icon="domain"         label="Department"   value={lecturer.department} />
            <InfoTile icon="briefcase-outline" label="Designation" value={lecturer.designation} />
            <InfoTile icon="email-outline"   label="Email"        value={lecturer.email} />
            <InfoTile icon="phone-outline"   label="Mobile"       value={lecturer.mobile || '—'} />
          </View>
          ...
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              ...
              <Text style={styles.cardTitle}>Assigned Courses</Text>
            ...
            {!lecturer.courses?.length ? (
              <Text style={styles.emptyCardText}>No courses assigned</Text>
            ) : (
          ...
          <View style={styles.card}>
            ...
            <InfoRow
              ...
              value={
                lecturer.createdAt
                  ? new Date(lecturer.createdAt).toLocaleDateString('en-US', {
            ...
            <InfoRow
              ...
              value={
                lecturer.lastLogin
                  ? new Date(lecturer.lastLogin).toLocaleDateString('en-US', {
                      year: 'numeric', month: 'long', day: 'numeric',
...
const styles = StyleSheet.create({
  ...
  profileCard: {
    ...
    alignItems: 'center',
    borderWidth: 1, borderColor: 'rgba(197,198,210,0.2)', gap: 6,
  },
  ...
  profileDesignation: {
    fontFamily: 'Manrope_600SemiBold', fontSize: 13, color: '#775a19',
  },
  profileId: {
